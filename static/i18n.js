...
const LOCALES = {
  ...
  folder_add_as_space_title: 'Add as Space?',
    ...
    settings_label_auxiliary_model: 'Auxiliary (Background) Model',
    settings_desc_auxiliary_model: 'Model for background tasks (compression, web extraction, titles, etc.). Set independently from the chat model so research models don\'t burn tokens on housekeeping.',
    settings_aux_none: 'None (use chat model)',
    'settings.fallback_models': 'Fallback Models',
    'settings.fallback_models_help': 'Automatic failover chain when the primary model fails (401, 429, timeout). The agent tries each model in order.',
  ...
  folder_add_as_space_title: 'Добавить как пространство?',
    ...
    settings_label_auxiliary_model: 'Фоновая модель',
    settings_desc_auxiliary_model: 'Модель для фоновых задач (сжатие контекста, извлечение из веба, заголовки и т.д.). Задаётся отдельно от модели чата, чтобы исследовательские модели не сжигали токены на обслуживание.',
    settings_aux_none: 'Нет (использовать модель чата)',
    'settings.fallback_models': 'Резервные модели',
    'settings.fallback_models_help': 'Цепочка автоматического переключения при ошибке основной модели (401, 429, timeout). Агент пробует каждую модель по порядку.',
    ...
    kanban_no_events: 'No events',
    kanban_no_runs: 'No runs',
    kanban_task_tree: 'Дерево задач',
    kanban_task_tree_hint: 'Нажмите на задачу для перехода · Подсвечена = текущая',
